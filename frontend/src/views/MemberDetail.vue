  ...
  <div class="member-detail">
    ...
    <div v-else-if="recommendations">
      <div class="member-header">
        <h2 class="member-name">{{ recommendations.DISC_D ? member.fullName : '' }}</h2>
      ...
      <div class="card">
        <h3 class="card-title">Совместимость</h3>
...
export default {
  ...
  methods: {
    async loadRecommendations() {
      try {
        const { teamId, memberId } = this.$route.params
        const response = await teamsApi.getMemberRecommendations(teamId, memberId)
